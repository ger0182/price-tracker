...
// 呼叫 Vercel Serverless Function（同網域，不需 API Key）
...
export default function PriceTracker() {
  ...
  useEffect(() => {
    try {
      ...
      setAutoEnabled(localStorage.getItem("price-tracker-auto") === "true");
    } catch {}
  }, []);

  const save = (history, price, checked) => {
    localStorage.setItem(STORAGE_KEY, JSON.stringify({ history, currentPrice: price, lastChecked: checked }));
  };
  ...
  const fetchPrice = useCallback(async () => {
    ...
    try {
      ...
      const entry = { date: now, price: data.price, original_price: data.original_price, in_stock: data.in_stock };
      const newHistory = [...priceHistory, entry].slice(-60);
      setPriceHistory(newHistory);
      setCurrentPrice(data.price);
      setLastChecked(now);
      setStatus("success");
      save(newHistory, data.price, now);
    } catch (e) {
      setStatus("error");
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (raw) {
        const d = JSON.parse(raw);
        const last = d.lastChecked ? new Date(d.lastChecked) : null;
  ...
  const latest = priceHistory.length ? priceHistory[priceHistory.length - 1] : null;
    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#0f0c29 0%,#302b63 50%,#24243e 100%)", fontFamily:"'Noto Sans TC',system-ui,sans-serif", padding:"24px 16px", color:"#e8e0f7" }}>
      ...
      <div style={{ maxWidth:640, margin:"0 auto" }}>
        ...
        <div style={{ textAlign:"center", marginBottom:28 }}>
          <div style={{ fontSize:11, letterSpacing:".2em", color:"#a78bfa", textTransform:"uppercase", marginBottom:6 }}>PChome 24h · 免費價格追蹤器</div>
        ...
        <div className="card fade-in" style={{ padding:24, marginBottom:16, position:"relative", overflow:"hidden" }}>
          <div style={{ position:"absolute", top:-40, right:-40, width:160, height:160, background:"radial-gradient(circle,rgba(139,92,246,.25) 0%,transparent 70%)", borderRadius:"50%", pointerEvents:"none" }}/>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
            <div>
              <div style={{ fontSize:12, color:"#9ca3af", marginBottom:8, letterSpacing:".05em" }}>目前售價</div>
              {currentPrice
                ? <div style={{ display:"flex", alignItems:"baseline", gap:8 }}>
                    <span className="mono" style={{ fontSize:46, fontWeight:600, color:"#c4b5fd", lineHeight:1 }}>${currentPrice.toLocaleString()}</span>
                    <span style={{ fontSize:14, color:"#6b7280" }}>TWD</span>
                    {priceDiff !== 0 && <span style={{ fontSize:13, color: priceDiff < 0 ? "#34d399" : "#f87171", fontWeight:600 }}>
                      {priceDiff < 0 ? "▼" : "▲"} {Math.abs(priceDiff)}
                    </span>}
                  </div>
                : <div style={{ fontSize:24, color:"#4b5563" }}>尚未查詢</div>}
              {lastChecked && <div style={{ fontSize:12, color:"#6b7280", marginTop:6 }}>最後更新：{fmt(lastChecked)}</div>}
            </div>
            <div style={{ textAlign:"right" }}>
              {lowest && priceHistory.length > 1 && <div style={{ marginBottom:10 }}>
              ...
              <div style={{ display:"inline-block", padding:"3px 12px", borderRadius:20, fontSize:11,
                background: latest?.in_stock !== false ? "rgba(52,211,153,.15)" : "rgba(239,68,68,.15)",
                border: `1px solid ${latest?.in_stock !== false ? "rgba(52,211,153,.3)" : "rgba(239,68,68,.3)"}`,
                color: latest?.in_stock !== false ? "#34d399" : "#f87171"
              }}>
                {latest?.in_stock !== false ? "● 有庫存" : "● 無庫存"}
              </div>
            </div>
          </div>
        </div>
        ...
        {priceHistory.length > 1 && (
          <div className="card fade-in" style={{ padding:20, marginBottom:16 }}>
            ...
            <svg width="100%" viewBox={`0 0 ${CW} ${CH}`} style={{ overflow:"visible" }}>
              ...
              {[0,.25,.5,.75,1].map(f => {
                const y = PT + (1-f) * (CH-PT-PB);
                return <g key={f}>
                  <line x1={PL} y1={y} x2={CW-PR} y2={y} stroke="rgba(255,255,255,.06)" strokeWidth="1"/>
                  <text x={PL-4} y={y+4} textAnchor="end" style={{ fontSize:9, fill:"#6b7280", fontFamily:"JetBrains Mono,monospace" }}>
                    {Math.round(minP + f * range)}
                  </text>
                </g>;
              })}
              {priceHistory.filter((_,i) => i % Math.max(1, Math.floor(priceHistory.length/5)) === 0).map(h => {
                const i = priceHistory.indexOf(h);
                return <text key={i} x={gx(i)} y={CH-4} textAnchor="middle" style={{ fontSize:9, fill:"#6b7280", fontFamily:"JetBrains Mono,monospace" }}>{fmtS(h.date)}</text>;
              })}
              ...
              {priceHistory.map((h, i) => <circle key={i} cx={gx(i)} cy={gy(h.price)} r="3.5"
                fill={h.price === minP ? "#34d399" : "#8b5cf6"} stroke="rgba(15,12,41,.8)" strokeWidth="1.5"/>)}
            </svg>
          </div>
        )}
        ...
        <div style={{ display:"flex", gap:10, marginBottom:16 }}>
          <button className="btn" onClick={fetchPrice} disabled={loading} style={{
            flex:1, padding:14, fontSize:14, color:"white",
            ...
            {loading
              ? <span style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}><span className="spin">⟳</span> 查詢中...</span>
              : "🔍 立即查詢價格"}
          </button>
          <a href={PRODUCT_URL} target="_blank" rel="noopener noreferrer" className="btn" style={{
            padding:"14px 16px", fontSize:14, background:"rgba(255,255,255,.08)",
            color:"#d1d5db", textDecoration:"none", display:"flex", alignItems:"center"
          }}>🛒 商品頁</a>
        </div>

        {/* Status */}
        {status === "success" && (
          <div className="fade-in" style={{ padding:"12px 16px", borderRadius:10, marginBottom:16, background:"rgba(52,211,153,.1)", border:"1px solid rgba(52,211,153,.2)", color:"#34d399", fontSize:13 }}>
            ✓ 查詢成功！目前售價 <strong className="mono">${currentPrice?.toLocaleString()}</strong> TWD
          </div>
        )}
        {status === "error" && (
          <div className="fade-in" style={{ padding:"12px 16px", borderRadius:10, marginBottom:16, background:"rgba(239,68,68,.1)", border:"1px solid rgba(239,68,68,.2)", color:"#f87171", fontSize:13 }}>
            ✗ {errorMsg}
          </div>
        )}
        ...
        <div className="card" style={{ padding:16, marginBottom:16 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div onClick={toggleAuto} style={{
              ...
              background: autoEnabled ? "#7c3aed" : "rgba(255,255,255,.1)",
              position:"relative", transition:"background .2s", border:"1px solid rgba(255,255,255,.15)"
            }}>
            ...
            <div style={{ fontSize:12, color: autoEnabled ? "#a78bfa" : "#6b7280" }}>
              {autoEnabled ? "✓ 已啟用 — 每天首次開啟頁面自動查詢" : "啟用後每天首次開啟頁面自動記錄價格"}
            </div>
          </div>
          <div style={{ marginTop:10, fontSize:11, color:"#4b5563", lineHeight:1.6 }}>
            💡 搭配瀏覽器書籤使用效果最佳。若需完全自動化，可用 <strong style={{color:"#6b7280"}}>UptimeRobot</strong> 每天定時呼叫此頁面。
          </div>
        </div>
          ...
          <div className="card" style={{ padding:20, marginBottom:16 }}>
            ...
            <div style={{ maxHeight:240, overflowY:"auto" }}>
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                <thead>
                  <tr style={{ color:"#6b7280", borderBottom:"1px solid rgba(255,255,255,.08)" }}>
                ...
                <tbody>
                  {[...priceHistory].reverse().map((h, i) => {
                    ...
                    return (
                      <tr key={i} className="rh" style={{ borderBottom:"1px solid rgba(255,255,255,.04)" }}>
                        <td style={{ padding:"7px 8px", color:"#9ca3af", fontFamily:"JetBrains Mono,monospace" }}>{fmt(h.date)}</td>
                        <td style={{ padding:"7px 8px", textAlign:"right", fontFamily:"JetBrains Mono,monospace", fontWeight:600, color: isLow ? "#34d399" : "#c4b5fd" }}>
                          ${h.price.toLocaleString()}{isLow && <span style={{ fontSize:10, marginLeft:4 }}>最低</span>}
                        </td>
                        <td style={{ padding:"7px 8px", textAlign:"right", color:"#6b7280", fontFamily:"JetBrains Mono,monospace", textDecoration:"line-through" }}>
                          {h.original_price ? `$${h.original_price.toLocaleString()}` : "—"}
                        </td>
                        <td style={{ padding:"7px 8px", textAlign:"center", color: h.in_stock ? "#34d399" : "#f87171" }}>
                          {h.in_stock ? "●" : "○"}
                        </td>
                      </tr>
                    );
        ...
        <div style={{ textAlign:"center", fontSize:11, color:"#4b5563", paddingBottom:16 }}>
          資料來源：PChome 24h 購物 · 完全免費，不需 API Key · 記錄存於本機瀏覽器
        </div>
      </div>
    </div>
  );
